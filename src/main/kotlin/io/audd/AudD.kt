...
public class AudD(
    ...
    /**
     * Custom-catalog upload is metered; an automatic retry on transport failure
     * could double-charge for the same audio fingerprinting. `maxAttempts = 1`
     * here is intentional and is NOT influenced by [maxRetries].
     */
    private val criticalPolicy: RetryPolicy get() = RetryPolicy(RetryClass.CRITICAL, maxAttempts = 1, backoffFactor = backoffFactor)
     ...
     */
    public val customCatalog: CustomCatalog by lazy { CustomCatalog(http, criticalPolicy) }
        ...
        val resp = try {
            retry(recognitionPolicy, shouldRetryByResponse = { shouldRetryResponse(it.httpStatus, RetryClass.RECOGNITION) }) {
                ...
                val data = part.data.toMutableMap().apply {
                    if (extraParameters != null) putAll(extraParameters)
    ...
    public suspend fun recognize(
        ...
        market: String? = null,
        extraParameters: Map<String, String>? = null,
    ): RecognitionResult? = recognize(Source.Url(url), returnMetadata, market, extraParameters)
    ...
    public suspend fun recognizeEnterprise(
        ...
        val extra = buildEnterpriseFields(
            formatReturn(returnMetadata), skip, every, limit, skipFirstSeconds, useTimecode, accurateOffsets,
        )
