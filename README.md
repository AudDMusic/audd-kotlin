...
Official Kotlin SDK for [AudD](https://audd.io) — music recognition from a short audio clip, a long audio file, or a live stream.
...
// In your webhook handler, given the parsed JSON `body: JsonObject`:
val payload = parseCallback(body)
when {
    payload.isResult -> for (entry in payload.result!!.results) {
        println("${entry.artist} — ${entry.title} score=${entry.score}")
    }
    payload.isNotification -> println("notification: ${payload.notification!!.notificationMessage}")
}
```
...
The Kotlin-idiomatic API is a `Flow<LongpollEvent>`:
...
fun main() = runBlocking {
    AudD.fromEnvironment().use { audd ->
        val category = audd.streams.deriveLongpollCategory(radioId = 42L)
        audd.streams.longpollFlow(category)
            .take(10)
            .collect { event ->
                when {
                    event.isResult       -> println("recognition: ${event.result}")
                    event.isNotification -> println("notification: ${event.notification}")
                    event.isTimeout      -> println("server-side timeout, looping…")
                }
            }
    }
}
...
`longpollFlow` advances `since_time` automatically across iterations, so each emitted `LongpollEvent` is strictly newer than the previous one. Cancel by breaking out of `collect` (e.g. `take(N)`) or by cancelling the surrounding coroutine — the loop is cooperative.
...
fun main() = runBlocking {
    ...
    LongpollConsumer(category = "abc123def").use { consumer ->
        consumer.events.take(10).collect { event -> println(event) }
    }
}
```
