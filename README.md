...
For longer audio files (full-length songs, short-form videos, podcasts, broadcasts, DJ sets), use `recognizeEnterprise(source, limit = ...)` — it returns a `List<EnterpriseMatch>`, one per song detected across the file.
...
The typed models cover what AudD documents. To read additional fields the server returns, go through `extras`:
...
For sending arbitrary form fields the typed parameters don't cover, pass an `extraParameters` map:

```kotlin
val result = audd.recognize(
    Source.FilePath("/tmp/snippet.wav"),
    returnMetadata = listOf("apple_music"),
    extraParameters = mapOf("my_custom_flag" to "1"),
)
```
...
fun Application.auddRoutes() {
    routing {
        post("/audd/callback") {
            when (val ev = parseCallback(call.receive<ByteArray>())) {
                is CallbackEvent.Match        -> log.info("${ev.match.song.artist} — ${ev.match.song.title}")
                is CallbackEvent.Notification -> log.info(ev.notification.notificationMessage)
            }
...
@RestController
class AudDCallbackController {
    @PostMapping("/audd/callback")
    fun handle(@RequestBody body: ByteArray): String {
        when (val ev = parseCallback(body)) {
            is CallbackEvent.Match        -> { /* persist ev.match */ }
            is CallbackEvent.Notification -> { /* log ev.notification */ }
        }
        return "ok"
    }
}
...
Javalin.create().post("/audd/callback") { ctx ->
    val ev = parseCallback(ctx.bodyAsBytes())
    // handle ev as above
    ctx.result("ok")
}.start(8080)
```
